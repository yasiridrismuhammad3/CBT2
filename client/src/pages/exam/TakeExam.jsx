...
const TakeExam = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col cbt-lock-user-select select-none">
      ...
      <header className="sticky top-0 z-40 bg-damale-navy-900/95 backdrop-blur-md border-b border-damale-navy-700 px-4 sm:px-8 py-3.5 flex items-center justify-between">

        {/* Left: Exam Title & Subject */}
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-damale-gold-500 to-amber-600 text-white font-black flex items-center justify-center text-sm shadow ring-2 ring-damale-gold-400">
            CBT
          </div>
          <div>
            ...
            <p className="text-[11px] font-semibold text-damale-gold-400 uppercase tracking-wider">
              {exam.subject?.name} • Student: {user?.fullName} ({user?.dsNumber})
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Live Timer Badge */}
          <div className={`px-4 py-2 rounded-xl flex items-center gap-2 border font-mono font-bold text-sm sm:text-base transition-colors ${timeLeftSeconds < 300
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-damale-navy-800 hover:bg-damale-navy-700 text-slate-300 transition"
          ...
          <button
            ...
            className="btn-gold text-xs py-2.5 px-4 font-bold uppercase tracking-wider hidden sm:flex"
          >
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-col justify-between space-y-6">
          ...
          <div className="glass-card p-6 bg-damale-navy-800/80 border border-damale-navy-700/80 space-y-4">

            <div className="flex items-center justify-between border-b border-damale-navy-700/60 pb-3">
              <span className="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-damale-gold-500/20 text-damale-gold-400 border border-damale-gold-500/30">
                Question {currentIndex + 1} of {totalQuestions}
              </span>

              <button
                onClick={() => toggleMarkForReview(currentQ._id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition ${markedForReview.includes(currentQ._id)
              ...
              {currentQ.questionImage && (
                <div className="mt-4 max-w-md">
                  <img
                    ...
                    className="rounded-xl border border-damale-navy-700 shadow-md"
                  />
                </div>
              )}
            </div>
              ...
              {currentQ.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border transition-all duration-200 flex items-center gap-4 ${isSelected
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs transition ${isSelected
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              ...
              className="btn-secondary text-sm py-3 px-6 disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            {currentIndex === totalQuestions - 1 ? (
              <button
                onClick={() => setShowSubmitModal(true)}
                className="btn-gold text-sm py-3 px-8 font-bold uppercase tracking-wider"
              >
        ...
        <div className="glass-card p-6 bg-damale-navy-800/80 border border-damale-navy-700/80 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px] font-semibold">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300">Unanswered ({totalQuestions - answeredCount})</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-purple-500"></div>
              <span className="text-slate-300">Marked ({markedForReview.length})</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded border-2 border-damale-gold-400"></div>
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl font-bold text-xs border transition-all flex items-center justify-center ${bgColor} ${isCurrent ? 'ring-2 ring-damale-gold-400 scale-110 font-extrabold z-10' : 'hover:scale-105'
                    }`}
                >
                  {idx + 1}
                </button>
          ...
          {/* Submit Mobile CTA */}
          <div className="pt-4 border-t border-damale-navy-700/60">
            <button
              onClick={() => setShowSubmitModal(true)}
